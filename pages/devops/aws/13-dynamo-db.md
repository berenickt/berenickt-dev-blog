...
- 데이터 스토리지는 테이블에서 사용한 디스크 공간(GB)당 시간당 요금을 부과하는데, GB당 월 0.27 USD
- 읽기 용량 유닛과 쓰기 용량 유닛 요금이 있다.
  - `1 읽기 용량 유닛`은 초당 최대 2건의 읽기 작업을 제공하며, 읽기 요청 유닛 100만 건당 0.271 USD
  - `1 쓰기 용량 유닛`은 초당 최대 1건의 쓰기 작업을 제공하며, 쓰기 요청 유닛 100만 건당 1.3556 USD
...
DynamoDB의 스칼라 데이터 형식은 다음과 같다.

- `Number` : 양수, 음수 또는 0을 표현하는 타입
- `String` : 문자열을 위한 타입
- `Binary` : 압축 텍스트를 위한 타입
- `Boolean` : Boolean을 위한 타입 ,
- `Null` : null값을 위한 타입

내포 속성이 있는 복잡한 구조를 표현하기 위한 `문서 형식`(내포 속성이 있는 복잡한 구조를 표현)이 있다.

- `List (L)` : 순서가 지정된 값 모음을 저장할 수 있음.
- `Мар (M)` : 정렬되지 않은 이름-값 쌍의 모음을 저장할 수 있음

여러 스칼라 값을 표현할 수 있는 `다중 값 형식` (여러 스칼라 값을 표현)이 있다.
...
- DynamoDB는 기존 파티션이 지원할 수 있는 한도를 초과하여 테이블의 할당된 처리량 설정을 늘리는 경우와,
- 기존 파티션 용량이 다 차서 추가 스토리지 공간이 필요한 경우에 테이블에 추가 파티션을 할당하게 된다.

---

### 4.2 DynamoDB 키

보통 데이터베이스에서 키라고 하면 Table내에서 각 Item을 구분하는 고유 식별자를 의미한다.
