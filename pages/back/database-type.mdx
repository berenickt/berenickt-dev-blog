...
예를 들어, 이름과 나이를 저장하고 싶으면, 그림처럼 저장하면 됩니다. 너무 간단해서 실용성이 없어보인다고요?
...
데이터를 하드디스크에 저장하지 않고, **일차적으로 RAM에다가 저장**을 해줍니다. 왜냐하면 RAM이 하드보다 몇 백배 빠르니까요. 그래서 redis를 어떤 식으로 활용하냐면, 메인 DB를 하나 두고, 거기서 사람들이 자주 쓰는 데이터들은 redis에 추가로 저장해둡니다.
...
**정규화**란 데이터 중복이 되면, 이를 다른 테이블의 데이터로 쪼개는 행위를 말합니다. 이것때문에 데이터 출력문법이 길고 복잡해집니다.
...
실패 시 Rollback(되돌리기)도 가능합니다. 그래서 입출력 속도보다 정확도가 매우 중요한 서비스라면 RDBMS를 사용합니다.
...
데이터를 입출력할 떄는 Graph Query Language를 사용합니다. 그래서 자료 간의 관계를 중점적으로 저장하고 싶을 떄 사용합니다.
...
관계형 DB와 다르게 어떤 데이터를 저장할지 미리 정할 필요도 없습니다. 예를 들어, 갑자기 연락처라는 넣어도 상관없습니다.
즉, 구조가 바뀌어도 신경안씁니다.
...
그리고 데이터를 저장할 떄, 시간 기록 기능이 있어서, 시계열 데이터를 저장하고, 분석할 떄, 이 DB를 쓰는 경우가 있습니다. (Netflix에서 이걸로 님 시청기록 분석함)
