...
# 참고로, 기존 Homebrew 경로는 Intel칩 기준이라, /opt 경로로 바꿔줘야 한다
## /opt 경로 바꾸기
...
```bash
brew install node
arch -arm64 brew install node # m1 맥북
...
rbenv install 2.7.5
rbenv global 2.7.5    # 장치에 기본 루비 버전을 아래 버전으로 설정
ruby --version
...
- 코코아팟은 오브젝트C나 스위프트로 개발된 오픈 라이브러리들을
- 내 프로젝트에 간편하게 확장시킬 수 있도록 도와주는 iOS용 프로그램
- CocoaPods는 Ruby로 구축되어있으며 gem을 사용하여 cocoapods를 설치해야 하는데,
- 이 경우 sudo(관리자 권한)으로 설치해주어야 한다.

> 💡 Mac M1 Architecture는 CocoaPods와 호환되지 않는다.
>
> ```bash
...
> ```
>
> - 만약 설치하면서 에러가 발생하면 다음 명령어를 실행.
> - 이 명령어는 ffi 패키지를 설치하여, 동적 연결된 라이브러리를 불러와,
>   - M1 arch에 적절한 pod install이 실행되도록 해준다.
...
$ react-native init --version 0.61.5 cliFirst
```
...
### 6.2 Android 시뮬레이터 구동

---

## 6. Expo 프로젝트 설치 & 시뮬레이터 구동
