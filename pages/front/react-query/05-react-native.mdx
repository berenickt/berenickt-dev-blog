...
시도해 볼 만한 서드파티 Flipper 플러그인이 있다 <br />
https://github.com/bgaleotti/react-query-native-devtools

시도해 볼 만한 서드파티 Reactotron 플러그인이 있다. <br />
...
React Query는 이미 웹 브라우저에서 재연결 시 자동 재설정을 지원한다. <br />
...
웹의 이벤트 리스너 대신에, React Native는 AppState module 통해 focus 정보를 제공한다. <br />
...
일부 상황들에서, React Native Screen이 다시 포커싱될 때, <br />
...
성능 문제를 포함한 일부 상황에서는, <br />
React Native 화면이 포커스를 벗어날 때, re-renders을 중지할 수 있다. <br />
이를 달성하기 위해, `NotifyOnChangeProps` 쿼리 옵션과 함께, <br />
`@react-navigation/native`의 `FocusEffect`를 사용할 수 있다.

이 커스텀 Hook은 () `NotifyOnChangeProps` 옵션을 제공한다. <br />
(화면이 초점을 벗어날 때마다 빈 배열을 반환하는) <br />
...
위 코드에서, `useFocusEffect`는 () 사용된다. <br />
(콜백조건으로 사용할 기준값을 변경하는데,)

이 인자(argument)가 () 보장하기 위해 참조를 감싼다.  <br />
