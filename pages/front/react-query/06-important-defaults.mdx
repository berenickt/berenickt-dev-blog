...
Out of the box(별도의 설치가 필요없이), TanStack Query는 적극적이지만 (제대로 동작되게) 설정된다.<br />
가끔 이런 기본설정값들ㅇ느 신규 사용자에게 학습/디버깅을 어렵게 만든다.<br />
...
> 이 동작을 변경하려면, `staleTime` 옵션을 사용하여 쿼리를 전역 및 쿼리별로 구성할 수 있습니다. <br />
...
(3) 더 이상 사용되지 않는 인스턴스인 useQuery, useInfiniteQuery 또는 query observers들의 Query 결과들은<br />
...
(5) 데이터를 가져오는데 실패한 query들은 3번 자동으로 재시도한다.<br />
(대표적인 뒷걸음질(backoff, 여기서는 오류)의 delay와 함께,) <br />
(UI에 오류를 찾아서 표시하기 전에,)
...
(6) 기본적으로 쿼리 결과는 구조적으로 공유되어 있다.<br />
(^만약 데이터가 실제로 변경되어 있는지 감지하기 위해서,)<br />
(데이터 변경을 감지하지 못한다면, 데이터 참조는 변화하지 않은채 유지된다.)<br />
(더 나은 값 안전화를 돕기 위해,)<br />
(useMemo 및 useCallback르 다루는데(regards),)

이 개념이 생소하게(foreign) 들린다면 걱정하지 않아도 된다! <br />
