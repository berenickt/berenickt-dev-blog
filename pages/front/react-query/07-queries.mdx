...
query는 (unique key와 결합된,) (비동기 데이터 소스에) 선언적 종속성이다. <br />
프로미스 기반의 method(GET, POST)로 서버로부터 data를 fetch할 수 있다. <br />
...
useQuery에서 반환된 쿼리 결과에는 () 포함되어 있다. <br />
...
`result` 객체에는 몇가지 생산성을 위한 아주 중요한 상태가 포함되어 있다. <br />
...
대부분의 query의 경우, 일반적으로 `isPending` 상태를 확인하기 충분하다. <br />
그런 다음, `isError` 상태를 확인하고, <br />
