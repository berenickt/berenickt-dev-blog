...
너가 직접 만든 에러를 사용하거나, 발생하지 않은 `Error`를 사용하려는 경우, <br />
...
그러나 이는 useQuery의 다른 모든 제네릭에 타입 추론이 동작하지 않는다는 단점이 있다. <br />
`Error`가 아닌 것을 던지는 건 좋은 습관으로 고려되지 않는다. <br />
그래서 AxiosError와 같은 하위 클래스가 있는 경우, <br />
...
TanStack Query v5를 사용하면 모든 것에 대해 글로벌 오류 유형을 설정할 수 있습니다. <br />
(^콜 사이드(call-sides)에 제네릭을 정의할 필요 없이,) <br />
...
글로벌 에러 타입을 등록하는 것과 마찬가지로 글로벌 `Meta` 유형도 등록할 수 있습니다. <br />
...
useQuery에 인라인 query 옵션을 입력하면, 알아서 타입 추론해준다. <br />
