...
본질적으로, TanStack Query는 query keys를 기반으로 query 캐싱을 관리한다. <br />
query keys는 top level의 배열이어야 하며, <br />
단일 문자열이 포함된 배열처럼 단순하거나 많은 문자열 및 중첩 객체 배열처럼 복잡할 수 있다. <br />
...
query에서 데이터를 유니크하게 설명하기 위해 추가 정보가 필요할 때, <br />
문자열이 있는 직렬화 가능한 객체를 원하는 수만큼 사용해서 설명할 수 있다. <br />
이는 다음과 같은 경우에 유용하다.
...
query keys는 query 함수에 대해 종속적인 역할을 한다는 것에 알아두세요.<br />
종속 변수를 query keys에 추가하면 query가 독립적으로 캐시되고, <br />
변수가 변경될 때마다, query가 자동으로 refetch됩니다.(`stableTime` 설정에 따라) <br />
